...
def get_classical_args():
    ...
    parser.add_argument('--temporal_policy', type=str, default=default_args['temporal_policy'], choices=['indices', 'num', 'time', 'period', 'period_t'])
...
def get_deep_args():  
    ...
    parser.add_argument('--model_type', type=str, 
                        default=default_args['model_type'],
                        choices=['mlp', 'mlp_plr', 'snn', 'dcn2', 
                                 'ftt', 'tabr', 'modernNCA', 'tabm', 
                                 'mlp_temporal', 'mlp_plr_temporal', 'dcn2_temporal', 'snn_temporal', 
                                 'ftt_temporal', 'tabr_temporal', 'modernNCA_temporal', 'tabm_temporal', 
                                 'mlp_modulated', 'mlp_plr_modulated', 'tabm_modulated',
                                 ])
...
def tune_hyper_parameters(args, opt_space, train_val_data, info, validate_option='holdout_last'):
    ...
    def objective(trial):
        ...
        if args.model_type in ['mlp_plr', 'mlp_plr_temporal', 'mlp_plr_modulated']:
            config['model']["num_embeddings"].setdefault('type', 'PLREmbeddings')
            config['model']["num_embeddings"].setdefault('lite', True)
...
def get_method(model):
    ...
    elif model == 'mlp_plr_modulated':
        from model.methods.mlp_plr_modulated import MLP_PLR_ModulatedMethod
        return MLP_PLR_ModulatedMethod
