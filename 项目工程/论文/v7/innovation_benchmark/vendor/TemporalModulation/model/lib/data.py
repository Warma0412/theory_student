...
def get_dataset_tabred(args):
    ...
    if 'n_num_features' not in info:
        info['n_num_features'] = N['train'].shape[1] if N is not None else 0
    if 'n_cat_features' not in info:
        info['n_cat_features'] = C['train'].shape[1] if C is not None else 0
    ...
    if args.validate_option == 'holdout_last':
        ...
        N_trainval = {"train": N_trainval[val_sample:], "val": N_trainval[:val_sample]} if N_trainval is not None else None
        C_trainval = {"train": C_trainval[val_sample:], "val": C_trainval[:val_sample]} if C_trainval is not None else None
    ...
    def temporal_policy_num(M):
        return M[:, None]

    def temporal_policy_time(M):
        timestamp_features = []
        for timestamp in M:
        ...
        return np.array(timestamp_features)

    def temporal_policy_period(M):
        ''' Follows the idea in https://github.com/PriorLabs/tabpfn-time-series.
        '''
        timestamp_features = []
        for timestamp in M:
            dt = datetime.fromtimestamp(timestamp, tz=timezone.utc)
        ...
        M_linear = (M - np.mean(M, axis=0)) / np.std(M, axis=0)
        return np.concatenate([M_linear[:, None], np.array(timestamp_features)], axis=1)
        
    if args.temporal_policy == 'indices':
        pass
    elif args.temporal_policy == 'num':
        N_trainval["train"] = np.concatenate(
            [N_trainval["train"], temporal_policy_num(M_trainval["train"])], axis=1)
        N_trainval["val"] = np.concatenate(
            [N_trainval["val"], temporal_policy_num(M_trainval["val"])], axis=1)
        N_test["test"] = np.concatenate(
            [N_test["test"], temporal_policy_num(M_test["test"])], axis=1)
        M_trainval["train"], M_trainval["val"], M_test["test"] = None, None, None
    elif args.temporal_policy == 'time':
        N_trainval["train"] = np.concatenate(
            [N_trainval["train"], temporal_policy_time(M_trainval["train"])], axis=1)
        N_trainval["val"] = np.concatenate(
            [N_trainval["val"], temporal_policy_time(M_trainval["val"])], axis=1)
        N_test["test"] = np.concatenate(
            [N_test["test"], temporal_policy_time(M_test["test"])], axis=1)
        M_trainval["train"], M_trainval["val"], M_test["test"] = None, None, None
    elif args.temporal_policy == 'period':
        N_trainval["train"] = np.concatenate(
            [N_trainval["train"], temporal_policy_period(M_trainval["train"])], axis=1)
        N_trainval["val"] = np.concatenate(
            [N_trainval["val"], temporal_policy_period(M_trainval["val"])], axis=1)
        N_test["test"] = np.concatenate(
            [N_test["test"], temporal_policy_period(M_test["test"])], axis=1)
        M_trainval["train"], M_trainval["val"], M_test["test"] = None, None, None
    elif args.temporal_policy == 'period_t':
        M_trainval["train"] = temporal_policy_period(M_trainval["train"])
        M_trainval["val"] = temporal_policy_period(M_trainval["val"])
        M_test["test"] = temporal_policy_period(M_test["test"])
    else:
    ...
    if args.temporal_policy == 'indices':
        ...
        info['M_mean'] = None
        info['M_std'] = None
        assert M_trainval["train"].ndim == 2 or M_trainval["train"] is None
        info['M_dim'] = M_trainval["train"].shape[1] if M_trainval["train"] is not None else 0
