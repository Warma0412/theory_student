...
def inspect(path, name):
    ...
    assert "1.8466" in text and "SCPL" in text, path
    assert "Semi" in text and "条件" in text, path
    assert not re.search(r"\bV[1-7]\b|上一版|前一版", text), path
    if name == "slides":
        assert len(doc) == json.loads((ROOT / "results/document_generation.json").read_text())["slides"]
        compact = re.sub(r"\s+", "", text)
        for expected in ("30组互补50%", "固定数据", "条件矩", "800份"):
...
def main():
    ...
    for name in ("论文", "开题报告", "开题汇报"):
        ...
        assert digest_file(final / f"{name}.tex") == digest_file(ROOT / f"latex工程/{name}/{name}.tex")
    for name, report in reports.items():
        log = Path(report["path"]).with_suffix(".log")
        content = log.read_text(errors="replace") if log.exists() else ""
        assert "LaTeX Error:" not in content
        assert "There were undefined citations" not in content
        report["template_warnings"] = [
            line for line in content.splitlines()
            if "Missing character:" in line or "Invalid UTF-8" in line or "Font Warning:" in line]
        report["template_warning_scope"] = "retained; figures, page numbers and scientific content visually checked"
