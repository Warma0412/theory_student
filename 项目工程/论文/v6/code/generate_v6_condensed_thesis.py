#!/usr/bin/env python3
"""Generate the standalone concise thesis with benchmark evidence."""
...
from generate_v6_thesis import build_context, markdown_table, render  # noqa: E402


OUTPUT_PDF = PAPER_DIR / "各版本汇总" / "论文_v6_精简版.pdf"
...
""".strip()
    ...
    if unresolved:
        raise ValueError(f"Unresolved condensed placeholders: {unresolved}")
    return text
...
def build_independent_markdown() -> str:
    ...
    selection = pd.read_csv(V6_DIR / "results" / "benchmark_selection_summary.csv")
    prediction = pd.read_csv(V6_DIR / "results" / "benchmark_prediction_metrics.csv")
    fair_stability = pd.read_csv(V6_DIR / "results" / "fair_stability_summary.csv")
    benchmark = selection.merge(
        ...
        fair_stability[
            ["method", "mean_jaccard", "ci95_lower", "ci95_upper"]
        ],
        on="method",
        how="left",
    )
    ...
    benchmark["主集重合"] = benchmark["overlap_with_knockoff"].map(
        lambda value: f"{int(value)}/12"
    )
    context["BENCHMARK_COMPACT_TABLE"] = markdown_table(
        benchmark,
        ["method", "主集重合", "稳定性", "RMSE"],
        ["方法", "与主集重合", "重抽样稳定性", "测试RMSE"],
    )
    ablation = pd.read_csv(V6_DIR / "results" / "component_ablation.csv")
    keep = (
    ...
    ablation = ablation.loc[keep].copy()
    ablation["Jaccard"] = ablation["jaccard_with_main"].map(
        lambda value: f"{value:.3f}"
    )
    context["ABLATION_COMPACT_TABLE"] = markdown_table(
        ablation,
        ["ablation", "setting", "repetitions", "selected_count", "Jaccard"],
        ["消融", "设定", "M", "严格数", "与主集Jaccard"],
    )
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="olist-v6-condensed-") as temporary:
        workdir = Path(temporary)
        markdown_path = workdir / "论文_v6_精简版.md"
        html_path = workdir / "论文_v6_精简版.html"
        pdf_path = workdir / "论文_v6_精简版.pdf"
        ...
        subprocess.run(
            [
                ...
                "--embed-resources",
                f"--css={css_path}",
                f"--resource-path={workdir}:{V6_DIR}:{V4_DIR}",
                "-o",
                str(html_path),
            ],
            cwd=workdir,
