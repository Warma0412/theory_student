#!/usr/bin/env python3
"""Generate the standalone thesis with benchmarks and component ablations."""
...
def build_context() -> dict[str, str]:
    ...
    benchmark_selection = pd.read_csv(
        RESULTS / "benchmark_selection_summary.csv"
    )
    benchmark_prediction = pd.read_csv(
        RESULTS / "benchmark_prediction_metrics.csv"
    )
    component_ablation = pd.read_csv(RESULTS / "component_ablation.csv")
    simulation_comparison = pd.read_csv(
        RESULTS / "simulation_method_comparison.csv"
    )
    fair_stability = pd.read_csv(RESULTS / "fair_stability_summary.csv")
    fair_paired = pd.read_csv(
    ...
    benchmark_selection["原生入选"] = benchmark_selection["native_count"].map(
        lambda value: f"{int(value)}"
    )
    benchmark_selection["同预算"] = benchmark_selection["matched_budget"].map(
        lambda value: f"{int(value)}"
    )
    benchmark_selection["重合"] = benchmark_selection[
        "overlap_with_knockoff"
    ].map(lambda value: f"{int(value)}")
    benchmark_selection["Jaccard"] = benchmark_selection[
        "jaccard_with_knockoff"
    ].map(lambda value: f"{value:.3f}")
    benchmark_selection = benchmark_selection.merge(
        fair_stability[
            [
                "method",
                "mean_jaccard",
    ...
    benchmark_selection["稳定性（95%CI）"] = benchmark_selection.apply(
        lambda row: (
    ...
    benchmark_prediction["RMSE"] = benchmark_prediction["rmse_log"].map(
        lambda value: f"{value:.3f}"
    )
    benchmark_prediction["MAE"] = benchmark_prediction["mae_log"].map(
        lambda value: f"{value:.3f}"
    )
    benchmark_prediction["R2"] = benchmark_prediction["r2_log"].map(
        lambda value: f"{value:.3f}"
    )
    benchmark_prediction["WAPE"] = benchmark_prediction["wape_raw"].map(
        fmt_percent
    )
    component_ablation["Jaccard"] = component_ablation[
        "jaccard_with_main"
    ].map(lambda value: f"{value:.3f}")
    component_ablation["中位频率"] = component_ablation[
        "median_frequency_selected"
    ].map(fmt_percent)
    simulation_comparison["平均FDP"] = simulation_comparison["mean_fdp"].map(
        lambda value: f"{value:.3f}"
    )
    simulation_comparison["超标概率"] = simulation_comparison[
        "probability_fdp_above_q"
    ].map(fmt_percent)
    simulation_comparison["平均Power"] = simulation_comparison[
        "mean_power"
    ].map(lambda value: f"{value:.3f}")
    simulation_comparison["平均发现数"] = simulation_comparison[
        "mean_discoveries"
    ].map(lambda value: f"{value:.2f}")
    best_benchmark = benchmark_prediction.loc[
        benchmark_prediction["method"].ne("全部31维")
    ].sort_values("rmse_log").iloc[0]
    knockoff_benchmark = benchmark_selection.loc[
        benchmark_selection["method"].eq("去随机化Knockoff")
    ].iloc[0]
    elastic_benchmark = benchmark_selection.loc[
        benchmark_selection["method"].eq("Elastic Net")
    ].iloc[0]
    xgb_benchmark = benchmark_selection.loc[
        benchmark_selection["method"].eq("XGBoost-SHAP")
    ].iloc[0]
    best_stability = fair_stability.sort_values(
        "mean_jaccard", ascending=False
    ).iloc[0]
    knockoff_vs_elastic = fair_paired.loc[
        fair_paired["comparison"].eq("去随机化Knockoff - Elastic Net")
    ].iloc[0]
    knockoff_vs_xgb = fair_paired.loc[
        fair_paired["comparison"].eq("去随机化Knockoff - XGBoost-SHAP")
    ].iloc[0]
    derand_sim = simulation_comparison.loc[
        simulation_comparison["method"].eq("去随机化Knockoff")
    ].iloc[0]
    single_sim = simulation_comparison.loc[
        simulation_comparison["method"].eq("单次Knockoff")
    ].iloc[0]
    ...
    context.update({
        ...
        "BENCHMARK_SELECTION_TABLE": markdown_table(
            benchmark_selection,
            [
                "method",
                "原生入选",
        ...
        "BENCHMARK_PREDICTION_TABLE": markdown_table(
            benchmark_prediction.sort_values("rmse_log"),
            ["method", "n_test", "RMSE", "MAE", "R2", "WAPE"],
            ["方法", "测试N", "RMSE(log)", "MAE(log)", "R2", "WAPE"],
        ),
        "COMPONENT_ABLATION_TABLE": markdown_table(
            component_ablation,
            [
                "ablation",
                "setting",
        ...
        "SIMULATION_COMPARISON_TABLE": markdown_table(
            simulation_comparison,
            ["method", "平均FDP", "超标概率", "平均Power", "平均发现数"],
            ["方法", "平均FDP", "FDP>0.20概率", "平均Power", "平均发现数"],
        ),
        "BEST_BENCHMARK_METHOD": str(best_benchmark["method"]),
        "BEST_BENCHMARK_RMSE": f"{best_benchmark['rmse_log']:.3f}",
        "KNOCKOFF_BENCHMARK_RMSE": (
            f"{benchmark_prediction.loc[benchmark_prediction['method'].eq('去随机化Knockoff'), 'rmse_log'].iloc[0]:.3f}"
        ),
        "KNOCKOFF_BENCHMARK_STABILITY": f"{knockoff_benchmark['mean_jaccard']:.3f}",
        "KNOCKOFF_STABILITY_CI": (
            f"[{knockoff_benchmark['ci95_lower']:.3f}, "
            f"{knockoff_benchmark['ci95_upper']:.3f}]"
        ),
        "ELASTIC_BENCHMARK_STABILITY": f"{elastic_benchmark['mean_jaccard']:.3f}",
        "ELASTIC_STABILITY_CI": (
            f"[{elastic_benchmark['ci95_lower']:.3f}, "
            f"{elastic_benchmark['ci95_upper']:.3f}]"
        ),
        "XGB_BENCHMARK_STABILITY": f"{xgb_benchmark['mean_jaccard']:.3f}",
        "BEST_STABILITY_METHOD": str(best_stability["method"]),
        "BEST_STABILITY": f"{best_stability['mean_jaccard']:.3f}",
        "KNOCKOFF_ALGORITHMIC_STABILITY": (
            f"{knockoff_benchmark['resampling_stability']:.3f}"
        ),
        "KNOCKOFF_VS_ELASTIC_DIFF": f"{knockoff_vs_elastic['mean_difference']:.3f}",
        "KNOCKOFF_VS_ELASTIC_CI": (
            f"[{knockoff_vs_elastic['ci95_lower']:.3f}, "
            f"{knockoff_vs_elastic['ci95_upper']:.3f}]"
        ),
        "KNOCKOFF_VS_XGB_DIFF": f"{knockoff_vs_xgb['mean_difference']:.3f}",
        "KNOCKOFF_VS_XGB_CI": (
            f"[{knockoff_vs_xgb['ci95_lower']:.3f}, "
            f"{knockoff_vs_xgb['ci95_upper']:.3f}]"
        ),
        "SIM_DERAND_FDP": f"{derand_sim['mean_fdp']:.3f}",
        "SIM_DERAND_POWER": f"{derand_sim['mean_power']:.3f}",
        "SIM_SINGLE_FDP": f"{single_sim['mean_fdp']:.3f}",
        "SIM_SINGLE_POWER": f"{single_sim['mean_power']:.3f}",
    })
    return context
...
def render(text: str, context: dict[str, str]) -> str:
    ...
    if unresolved:
        raise ValueError(f"Unresolved thesis placeholders: {unresolved}")
    return text
...
论文/v1/.venv/bin/python 论文/v5/code/generate_v6_thesis.py
```
...
def run(command: list[str]) -> None:
    subprocess.run(command, check=True, cwd=V6_DIR)


def main() -> None:
    copy_base_figures()
    text = build_markdown()
    markdown_path = V6_DIR / "论文_v6.md"
    summary_dir = PAPER_DIR / "各版本汇总"
    summary_dir.mkdir(parents=True, exist_ok=True)
    root_markdown_path = summary_dir / "论文_v6_最终版.md"
    docx_path = V6_DIR / "论文_v6.docx"
    html_path = V6_DIR / "论文_v6.html"
    pdf_path = V6_DIR / "论文_v6.pdf"
    reference_path = V6_DIR / "reference.docx"
    css_path = V6_DIR / "thesis.css"

    markdown_path.write_text(text, encoding="utf-8")
    root_markdown_path.write_text(
        text.replace("](figures/", "](v6/figures/"),
        encoding="utf-8",
    )
    ...
    document.core_properties.subject = (
        "Model-X Knockoff；统一基准比较；组件消融；多粒度稳健性"
    )
