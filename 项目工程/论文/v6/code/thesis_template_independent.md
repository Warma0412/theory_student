...
为检验主方法相对于标准选择方法的增量价值，本文在测试期之前完成全部选维，并以相同12项预算比较Elastic Net、稳定性选择、影子变量树模型和XGBoost-SHAP。{{BEST_BENCHMARK_METHOD}}取得最低测试RMSE {{BEST_BENCHMARK_RMSE}}，去随机化Knockoff的RMSE为{{KNOCKOFF_BENCHMARK_RMSE}}。在30组完全相同的70%卖家子样本中，{{BEST_STABILITY_METHOD}}的平均集合稳定性最高，为{{BEST_STABILITY}}；去随机化Knockoff为{{KNOCKOFF_BENCHMARK_STABILITY}}，其与Elastic Net的配对差异置信区间包含0。在20轮真实协方差模拟中，去随机化Knockoff平均FDP为{{SIM_DERAND_FDP}}、Power为{{SIM_DERAND_POWER}}；单次Knockoff平均FDP为{{SIM_SINGLE_FDP}}、Power为{{SIM_SINGLE_POWER}}。结果表明预测最优、选择稳定和错误率控制是不同目标。
...
Under a common pre-test selection window and a matched budget of 12 variables, {{BEST_BENCHMARK_METHOD}} achieves the lowest test RMSE of {{BEST_BENCHMARK_RMSE}}, whereas derandomized knockoffs obtain an RMSE of {{KNOCKOFF_BENCHMARK_RMSE}}. Across 30 paired subsamples containing the same 70% of sellers for every method, {{BEST_STABILITY_METHOD}} has the highest mean set stability of {{BEST_STABILITY}}, while derandomized knockoffs obtain {{KNOCKOFF_BENCHMARK_STABILITY}} and are not distinguishable from Elastic Net by the paired confidence interval. In 20 covariance-preserving simulations, derandomized knockoffs attain a mean FDP of {{SIM_DERAND_FDP}} and a power of {{SIM_DERAND_POWER}}. These comparisons distinguish predictive accuracy, selection stability, and error-rate control rather than treating them as interchangeable objectives.
...
5. 在统一测试期前选维窗口和相同变量预算下比较五种标准方法，并通过S矩阵、边际变换、统计量和重复次数消融识别主流程各组件的作用。
...
基准比较遵循“同一信息集、同一截止时间、同一预测器、同一变量预算”的原则。所有方法仅使用2018年4月及以前的数据完成选维，以12项为匹配预算。比较对象包括去随机化Knockoff、Elastic Net、稳定性选择、影子变量树模型和XGBoost-SHAP。各方法选出的12项变量均输入相同参数的XGBoost，并在2018年5月至7月测试集上评价。选择稳定性另采用30组配对的卖家子样本：每组对所有方法使用完全相同的70%卖家，保留入选卖家的全部月份，在子样本内完整重算各方法Top-12，并计算其与各自全样本Top-12的Jaccard。去随机化Knockoff在每个子样本内聚合60轮。报告平均Jaccard、标准差和95%置信区间。固定全数据仅更换仿制变量所得的{{KNOCKOFF_ALGORITHMIC_STABILITY}}只作为算法随机性诊断，不再与样本稳定性混为一列。
...
{{BENCHMARK_SELECTION_TABLE}}

Elastic Net、稳定性选择和影子变量树模型均与Knockoff重合10项，XGBoost-SHAP重合8项。统一子抽样实验中，Elastic Net的平均稳定性为{{ELASTIC_BENCHMARK_STABILITY}}{{ELASTIC_STABILITY_CI}}，去随机化Knockoff为{{KNOCKOFF_BENCHMARK_STABILITY}}{{KNOCKOFF_STABILITY_CI}}，两者配对均值差为{{KNOCKOFF_VS_ELASTIC_DIFF}}，置信区间{{KNOCKOFF_VS_ELASTIC_CI}}包含0；XGBoost-SHAP为{{XGB_BENCHMARK_STABILITY}}。因此，Knockoff与Elastic Net在当前协议下均较稳定，不能宣称Knockoff单独最优；Knockoff相对XGBoost-SHAP的配对差异为{{KNOCKOFF_VS_XGB_DIFF}}，置信区间为{{KNOCKOFF_VS_XGB_CI}}。

**表5-10 同预算集合的统一XGBoost测试结果**

{{BENCHMARK_PREDICTION_TABLE}}

{{BEST_BENCHMARK_METHOD}}取得最低RMSE {{BEST_BENCHMARK_RMSE}}，去随机化Knockoff为{{KNOCKOFF_BENCHMARK_RMSE}}。两者差异很小，但目标不同：前者优化预测贡献，后者提供FDR框架下的条件发现。本文据此不宣称Knockoff在预测精度上全面领先。

![图5-6 配对卖家子抽样的集合稳定性](figures/fig22_fair_stability.png)

### 5.4.2 主流程组件消融

**表5-11 Copula、S矩阵、统计量与重复次数消融**
...
![图5-7 重复次数消融](figures/fig20_repetition_ablation.png)

### 5.4.3 统一模拟比较

**表5-12 真实协方差模拟的FDP与Power**

{{SIMULATION_COMPARISON_TABLE}}

在20轮、10个真信号的模拟中，去随机化Knockoff平均FDP为{{SIM_DERAND_FDP}}、Power为{{SIM_DERAND_POWER}}；单次Knockoff平均FDP为{{SIM_SINGLE_FDP}}、Power为{{SIM_SINGLE_POWER}}。Elastic Net和影子变量树模型均保持高Power，但平均FDP明显偏高；稳定性选择在该设定下也取得较低FDP，但不提供与Model-X相同的有限样本目标保证。

![图5-8 模拟FDP与Power比较](figures/fig21_simulation_fdp_power.png)

### 5.4.4 面板结构、业务组与非线性结果

月度卖家均值—离差分层结果见下表。卖家间层在0.20水平入选{{V4_BETWEEN_COUNT}}项，卖家内层为{{V4_WITHIN_COUNT}}项。整表信号主要对应卖家长期经营位置差异；当前19个月的面板不足以支持非空的卖家内严格集合。
...
**表5-14 月度业务组Knockoff结果**

{{V4_GROUP_TABLE}}

![图5-9 聚类分层与业务组稳定性](figures/fig12_cluster_group.png)
...
![图5-10 轨迹聚合平均W](figures/fig14_grip_importance.png)
...
![图5-11 周度GMV变化](figures/fig15_weekly_gmv.png)
...
![图5-12 周度单轮入选频率](figures/fig16_weekly_selection.png)
...
![图5-13 月度与周度入选频率](figures/fig17_monthly_weekly_frequency.png)
...
![图5-14 周度时间外预测比较](figures/fig18_weekly_prediction.png)
...
统一预算比较表明，不存在对所有评价目标都占优的单一方法。XGBoost-SHAP以{{BEST_BENCHMARK_RMSE}}取得最低测试RMSE，去随机化Knockoff为{{KNOCKOFF_BENCHMARK_RMSE}}，差值仅为0.008；在统一卖家子抽样下，两者的平均集合稳定性分别为{{XGB_BENCHMARK_STABILITY}}和{{KNOCKOFF_BENCHMARK_STABILITY}}。Elastic Net以{{ELASTIC_BENCHMARK_STABILITY}}取得最高均值，但与Knockoff的配对差异区间包含0。因此，响应驱动的树模型排序更接近纯预测目标，Elastic Net和Knockoff的集合更稳定，而Knockoff另提供Model-X框架下的目标FDR解释。

模拟结果进一步说明，预测能力、筛选功效与错误发现比例不能混为一个指标。去随机化Knockoff与单次Knockoff的平均Power均为{{SIM_DERAND_POWER}}，但平均FDP分别为{{SIM_DERAND_FDP}}和{{SIM_SINGLE_FDP}}；Elastic Net和影子变量树模型虽然保持高Power，却通过选择更多变量承担了较高FDP。稳定性选择在当前模拟中取得较低FDP，说明它是有竞争力的经验基线，但其阈值含义与Model-X有限样本FDR目标不同，不能仅凭单一模拟场景宣布一种方法普遍优越。
...
5. 在测试期前选维、相同12项预算和统一XGBoost预测器下，XGBoost-SHAP以{{BEST_BENCHMARK_RMSE}}取得最低测试RMSE，去随机化Knockoff为{{KNOCKOFF_BENCHMARK_RMSE}}。30组配对卖家子抽样中，Elastic Net与Knockoff的平均稳定性分别为{{ELASTIC_BENCHMARK_STABILITY}}和{{KNOCKOFF_BENCHMARK_STABILITY}}，差异区间包含0。20轮模拟中，去随机化Knockoff平均FDP由单次方法的{{SIM_SINGLE_FDP}}降至{{SIM_DERAND_FDP}}，且Power保持{{SIM_DERAND_POWER}}。基准与消融共同表明，本文方法的主要价值是预测性能接近最优、集合较稳定且具有错误率目标解释，而非宣称在所有指标上领先。
