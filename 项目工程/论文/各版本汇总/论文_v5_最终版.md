...
研究使用Olist巴西电商公开数据，在订单、商品明细、支付、评价、商品、卖家、顾客、地理位置和营销漏斗的自然粒度上先聚合后连接。月度主分析形成14,677个卖家-月观测、2,793个卖家和19个月；周度稳健性分析形成33,190个卖家-周观测、2,834个卖家和83周。本文构造32项候选维度，排除一项零方差变量后，使用31项本期特征预测下一期`log(1+GMV)`。该目标避免件数、订单数和均价对同周期GMV的机械解释。
...
顾客数量使用`customer_unique_id`去重，而非订单级`customer_id`。样本包含96,478笔已送达订单和110,197行已送达商品明细。处理后的GMV与原始商品明细逐笔对账一致，月度和周度面板主键重复数均为零。
...
![图3-1 月度GMV变化](v5/figures/fig01_monthly_gmv.png)

![图3-2 次月对数GMV分布](v5/figures/fig02_target_distribution.png)
...
月度面板包含14,677行，样本GMV为12.342百万雷亚尔，最大缺失率为1.49%。面板主键无重复，处理后GMV与原始已送达商品明细一致。月度面板SHA-256为`7212bf462a8b23ba6455bed5fe7354481b43d3e3a0f633a13ced1a97c3046a1e`。
...
![图5-1 生成器联合交换性诊断](v5/figures/fig13_exchangeability.png)
...
![图5-2 月度主分析平均e-value](v5/figures/fig03_knockoff_evalues.png)

![图5-3 月度主分析单轮入选频率](v5/figures/fig04_selection_frequency.png)
...
![图5-4 月度时间外预测比较](v5/figures/fig10_predictive_models.png)
...
![图5-5 XGBoost SHAP重要性](v5/figures/fig06_xgboost_shap.png)
...
![图5-6 聚类分层与业务组稳定性](v5/figures/fig12_cluster_group.png)
...
![图5-7 轨迹聚合平均W](v5/figures/fig14_grip_importance.png)
...
![图5-8 周度GMV变化](v5/figures/fig15_weekly_gmv.png)
...
![图5-9 周度单轮入选频率](v5/figures/fig16_weekly_selection.png)
...
![图5-10 月度与周度入选频率](v5/figures/fig17_monthly_weekly_frequency.png)
...
![图5-11 周度时间外预测比较](v5/figures/fig18_weekly_prediction.png)
...
1. 自然粒度先聚合后连接能够避免支付、评价和商品明细多对多连接造成的金额膨胀。月度主面板包含14,677个卖家-月，周度面板包含33,190个卖家-周；下一周期目标避免了同周期GMV定义关系。
2. 随机化秩高斯Copula显著改善混合边际匹配。60轮Knockoff的e-value聚合在0.20水平形成14项月度确认集合，参数化展示规则推荐$K=10$，测试RMSE为2.385。
3. 周度分析形成18项集合，其中11项与月度重合，Jaccard系数为0.524。周度18项XGBoost的RMSE为2.421，与全31维模型基本一致。月度适合确认，周度适合稳健性和短期预警。
...
|---|---|
| 随机种子 | 20260823 |
| 月度面板SHA-256 | 7212bf462a8b23ba6455bed5fe7354481b43d3e3a0f633a13ced1a97c3046a1e |
| 周度面板SHA-256 | bef6bd6f902d076de1e3dcda5234acf02163ba8d650e34c43b2d7972b22a48ff |
| Python | 3.9.6 |
| NumPy | 2.0.2 |
| pandas | 2.3.3 |
| scikit-learn | 1.6.1 |
