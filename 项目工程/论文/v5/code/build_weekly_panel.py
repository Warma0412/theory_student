#!/usr/bin/env python3
"""Build the Olist seller-week panel for the V5 granularity robustness study.
...
V5_DIR = CODE_DIR.parent
PROJECT_DIR = V5_DIR.parents[1]
MAIN_DIR = PROJECT_DIR / "原始数据" / "archive"
FUNNEL_DIR = PROJECT_DIR / "原始数据" / "archive (漏斗维度补充)"
OUT_DIR = V5_DIR / "data_processed"
RESULTS_DIR = V5_DIR / "results"
LOG_DIR = V5_DIR / "logs"

PRIMARY_START = pd.Timestamp("2017-01-02")
PRIMARY_FEATURE_END = pd.Timestamp("2018-07-30")
...
def build_panel() -> tuple[pd.DataFrame, dict]:
    ...
    # ISO-style Monday-start weeks make the 83 weekly periods reproducible.
    orders["week"] = (
    ...
    item_agg = enriched.groupby(["seller_id", "week"], as_index=False).agg(
        gmv=("price", "sum"),
    ...
    state_counts = (
        enriched.groupby(["seller_id", "week", "customer_state"], as_index=False)
        .size()
        .groupby(["seller_id", "week"], as_index=False)["size"]
        .max()
        .rename(columns={"size": "dominant_customer_state_items"})
    )
    item_agg = item_agg.merge(state_counts, on=["seller_id", "week"], how="left", validate="1:1")
    item_agg["dominant_customer_state_share"] = (
        item_agg["dominant_customer_state_items"] / item_agg["item_count"]
    )

    seller_orders = enriched[["seller_id", "week", "order_id"]].drop_duplicates()
    ...
    order_agg = seller_orders.groupby(["seller_id", "week"], as_index=False)[order_feature_cols].mean()
    panel = item_agg.merge(order_agg, on=["seller_id", "week"], how="left", validate="1:1")
    ...
    future = panel[["seller_id", "week", "gmv", "order_count"]].copy()
    future["week"] = future["week"] - pd.Timedelta(days=7)
    future = future.rename(columns={"gmv": "gmv_next_week", "order_count": "orders_next_week"})
    panel = panel.merge(future, on=["seller_id", "week"], how="left", validate="1:1")
    panel[["gmv_next_week", "orders_next_week"]] = panel[["gmv_next_week", "orders_next_week"]].fillna(0)
    panel["log_gmv"] = np.log1p(panel["gmv"])
    panel["log_gmv_next_week"] = np.log1p(panel["gmv_next_week"])
    panel["aov_next_week"] = np.divide(
        panel["gmv_next_week"],
        panel["orders_next_week"],
        out=np.zeros(len(panel), dtype=float),
        where=panel["orders_next_week"].to_numpy() > 0,
    )
    panel["log_aov_next_week"] = np.log1p(panel["aov_next_week"])
    panel["year_week"] = panel["week"].dt.strftime("%G-W%V")
    panel = panel.loc[
        panel["week"].between(PRIMARY_START, PRIMARY_FEATURE_END)
    ].sort_values(["week", "seller_id"]).reset_index(drop=True)

    duplicate_keys = int(panel.duplicated(["seller_id", "week"]).sum())
    if duplicate_keys:
        raise ValueError(f"Panel has {duplicate_keys} duplicated seller-week keys")
    if not np.isclose(panel["gmv"].sum(), enriched.loc[
        enriched["week"].between(PRIMARY_START, PRIMARY_FEATURE_END), "price"
    ].sum()):
    ...
    audit = {
        ...
        "panel_start": panel["year_week"].min(),
        "panel_end": panel["year_week"].max(),
...
def save_outputs(panel: pd.DataFrame, audit: dict) -> None:
    ...
    panel_path = OUT_DIR / "seller_week_panel.csv"
