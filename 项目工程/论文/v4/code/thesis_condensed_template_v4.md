...
subtitle: Olist聚类、组级与深度诊断精简介绍版（v4）
author: |
  学校：【待填写】  
  学院：【待填写】  
  专业：【待填写】  
...
V4进一步实跑聚类均值—离差Knockoff、十二业务组Group Knockoff、最坏swap深度生成、深度核MMD和GRIP2式轨迹聚合重要性。0.20水平卖家间层入选{{V4_BETWEEN_COUNT}}项、卖家内层{{V4_WITHIN_COUNT}}项；组级为{{V4_GROUP_Q20_COUNT}}组。最坏swap生成器平均KS为{{V4_DEEP_KS}}，但深度核拒绝率{{V4_DEEP_KERNEL_REJECT}}、分类AUC {{V4_DEEP_AUC}}，仍被否决。GRIP2式统计量0.20水平{{V4_GRIP_Q20_COUNT}}项、0.30水平{{V4_GRIP_Q30_COUNT}}项。
...
**关键词：** 电商 GMV；聚类Knockoff；Group Knockoff；深度核MMD；GRIP2；FDR
...
聚类情景把每个特征分为卖家均值和卖家内离差，在两个层级分别重复Knockoff。组级情景把31项预先划为12个业务组，以组内真实/仿制Lasso系数的$L_2$范数差构造组级W。

深度生成器加入可微最坏swap对手，边际校准仅在80%训练子集拟合。诊断新增训练/检验分离的深度核MMD：核网络在训练子集最大化两样本差异，再在检验子集执行199次置换。GRIP2式分支在二维正则网格与优化轨迹上累计配对重要性，并单独验证反对称性。
...
卖家间层0.20水平入选{{V4_BETWEEN_COUNT}}项，卖家内层为{{V4_WITHIN_COUNT}}项。主信号更多反映卖家长期位置差异，而非短期月内变化。

**表4-8 组级结果**

{{V4_GROUP_TABLE}}

十二组在0.20水平无严格发现，0.30水平有{{V4_GROUP_Q30_COUNT}}组：{{V4_GROUP_Q30_LIST}}，反映低组数下的离散门槛。

**表4-9 深度生成联合诊断**

{{V4_DIAGNOSTIC_TABLE}}

训练集校准消除了验证分位数复用，但V2与V4深度生成器均被深度核MMD和分类器拒绝；V4分支继续不作FDR声明。

**表4-10 GRIP2式重要性**

{{V4_GRIP_TABLE}}

反对称误差为{{V4_GRIP_ERROR}}；0.20水平无发现，0.30水平入选{{V4_GRIP_Q30_COUNT}}项：{{V4_GRIP_Q30_LIST}}。
...
本文件为v4精简介绍版。V4保留V1—V3全部真实结果，并新增聚类分层、组级FDR、最坏swap生成、深度核诊断、训练集校准和轨迹聚合重要性。
