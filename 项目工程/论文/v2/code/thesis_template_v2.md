...
subtitle: 基于Olist卖家月度面板的去随机化Model-X Knockoff、深度生成诊断与可解释机器学习（v2）
author: |
  学校：【待填写】  
  学院：【待填写】  
  专业：【待填写】  
...
本文档为研究生学位论文 v2 完整稿。原始交易数据来自公开的 Olist Brazilian E-Commerce Public Dataset 与 Marketing Funnel by Olist；V1 的数据、统计基线和图表保存在 `论文/v1`，V2 的深度学习代码、模型、逐轮结果、诊断和图表保存在 `论文/v2`。封面中的学校、学院、专业、姓名、学号和导师信息需由作者按实际情况填写。正式提交前，还应以所在学校当年发布的学位论文模板替换封面、授权声明、页眉和参考文献细则。
...
本文的主要贡献是：第一，将错误发现率控制引入电商看板维度治理；第二，通过自然粒度聚合和前瞻目标规避数据膨胀与目标泄漏；第三，建立“深度生成、交换性诊断、深度重要性、时间外预测”分离验证的证据链，并公开保留未通过诊断的结果；第四，把严格 e-BH、稳定性、预测解释和固定效应置于统一分层框架；第五，提供从 11 张原始 CSV 到面板、模型、结果表、图和论文的完整可复现流程。

**关键词：** 电商 GMV；Model-X Knockoff；深度 Knockoff；错误发现率；e-value；深度表格学习
...
**Key words:** E-commerce GMV; Model-X knockoffs; deep knockoffs; false discovery rate; e-values; deep tabular learning
...
研究流程包括七个阶段。第一阶段审计 11 张原始表，在自然粒度上先聚合。第二阶段构造卖家-月份面板和次月目标，完成主键、时间、缺失和 GMV 对账。第三阶段生成 Copula-MVR Knockoff，并以 Lasso、重复生成和 e-value 聚合实施确认性 FDR 控制。第四阶段训练深度生成器，在独立保留集上执行多指标交换性诊断，并按预设规则决定其是否有资格进入推断。第五阶段在有效 Copula 负对照上训练配对竞争 MLP，并做反对称性单元测试。第六阶段按自然月份比较树模型、传统 MLP 和残差表格 MLP，并以 SHAP 和固定效应补充解释。第七阶段把不同证据转化为看板分层建议。

## 1.5 可能创新

第一，本文将研究目标从重要性排序转化为控制看板维度发现错误。第二，采用次月 GMV 避免同月定义泄漏。第三，构建现代 PyTorch 深度 Knockoff 实验，并将 swap-MMD、分类器两样本检验、KS、协方差、覆盖率和可重构性联合用于模型准入。第四，提出满足配对交换反对称性的深度 MLP 重要性，并以数值单元测试验证。第五，固定 V1 已选惩罚系数和 e-value 后再考察 V2 分支及事后阈值路径，减少新增模型的调参自由度。第六，公开否决未通过诊断的深度生成器，把“AI 模型能否被证伪”纳入论文贡献。

## 1.6 论文结构

第2章梳理 FDR、Knockoff、深度生成、e-value、深度表格学习和面板模型文献；第3章说明数据与变量；第4章给出确认性基线和 V2 深度实验设计；第5章报告完整实证与诊断结果；第6章讨论指标治理和 AI 的适用边界；第7章总结并提出后续研究方向。
...
第一条命令读取全量原始 CSV、构造面板并完成 GMV 对账；第二条命令运行 V1 全部统计基线；第三条命令运行 V2 深度生成、诊断、配对 MLP 与残差 MLP；第四条命令从机器可读结果生成 V2 Markdown、DOCX、HTML 和 PDF。V1 每次 Knockoff 的 W 保存在 `论文/v1/results/knockoff_runs`，V2 逐轮结果、模型和日志分别保存在 `论文/v2/results`、`models` 和 `logs`。
