#!/usr/bin/env python3
"""Generate the concise V2 PDF from verified V1 and V2 results."""
...
TEMPLATE = Path(__file__).with_name("thesis_condensed_template_v2.md")
OUTPUT_PDF = V2_DIR.parent / "论文_v2_精简版.pdf"
...
def build_markdown() -> str:
    ...
    if controls:
        raise ValueError("V2 condensed thesis contains control characters")
    return text
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="olist-v2-condensed-") as temporary:
        workdir = Path(temporary)
        markdown_path = workdir / "论文_v2_精简版.md"
        html_path = workdir / "论文_v2_精简版.html"
        pdf_path = workdir / "论文_v2_精简版.pdf"
        ...
        subprocess.run(
            [
                ...
                "--embed-resources",
                f"--css={css_path}",
                f"--resource-path={workdir}:{V2_DIR}:{V1_DIR}",
                "-o",
                str(html_path),
            ],
            cwd=workdir,
    ...
    print(
        ...
        f"({len(text)} characters, {OUTPUT_PDF.stat().st_size} bytes)"
    )
